...
export function registerTreeLevelCommand(
  ...
  return vscode.commands.registerCommand('headingNavigator.adjustTreeLevel', async () => {
    ...
    const value = await vscode.window.showInputBox({
      prompt: 'Enter a heading level to expand (0=collapse all, empty=expand all).',
      placeHolder: 'Empty for expand all, 0 to collapse, 1-6 for a specific depth',
    ...
    if (trimmed === '') {
      ...
      void vscode.window.showInformationMessage('Expanded all heading levels.');
      return;
    }
...
async function expandToLevel(
  ...
  const queue: Array<{ node: HeadingNode; depth: number }> = roots.map((node) => ({ node, depth: 1 }));
...
async function collapseAllTreeItems(): Promise<void> {
  try {
    await vscode.commands.executeCommand('workbench.actions.treeView.headingNavigator.headingTree.collapseAll');
  } catch (error) {
